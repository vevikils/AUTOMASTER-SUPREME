...
$zipPath = Join-Path $workspace "AUTOMASTER_SUPREME_v3.0.0_Windows.zip"
...
Copy-Item -Path "$workspace\vst3-plugin\build\AutomasterSupreme_artefacts\Release\VST3\AUTOMASTER SUPREME 3.vst3" -Destination (Join-Path $pkgDir "VST3") -Recurse -Force
Copy-Item -Path "$workspace\vst3-plugin\build\AutomasterSupreme_artefacts\Release\Standalone\AUTOMASTER SUPREME 3.exe" -Destination (Join-Path $pkgDir "Standalone") -Force
